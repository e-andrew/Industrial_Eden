...
### Open Numbers
Це спільнота розподільників даних, які гармонізують статистику, щоб зробити її більш придатною для світогляду, заснованого на фактах. 
Концептуальна модель реалізації Open Numbers представлена у Data Description Format.
...
Датапоінт:
-   Дані в DDF зберігаються в парах ключ-значення, які називаються DataPoints. Ключ складається з двох або більше
розмірів, тоді як значення складається з одиного показника. 

Cутність:
-   Кожна сутність представлена унікальним ідентифікатором у стовпці домену сутності. Цей ідентифікатор унікальний, але лише в межах домену сутності. Ідентифікатори сутності можуть містити лише малі літерно-цифрові символи та підкреслення.
-   Набір сутності - це сукупність сутностей у домені сутності. Домен сутності - це сам набір сутності, що містить усі сутності в цьому домені.

Метаданні:
-   Метадані - це дані про дані. Вони можуть бути настільки детальними, як опис одного конкретного числа в наборі даних, одного значення точки даних. Тому в DDF ми визначаємо метадані як властивість однієї пари даних ключ-значення в колекції.

Концепції:
-   Концепції - це заголовки таблиць у будь-якому місці набору даних DDF.
-   Властивості поняття - це поняття, які дають інформацію про поняття.
-   Ідентифікатори концепції, рядки під заголовком стовпця "концепція", відповідають тим же правилам, що і ідентифікатори сутності: Ідентифікатори концепції можуть містити лише малі буквено-цифрові символи та підкреслення.

Синоніми:
-   Синонім - це рядок, який може визначити концепцію або сутність, що є у наборі даних. Колекція синонімів містить ідентифікатори сутності та поняття з їх синонімами
-   Синоніми використовуються для пошуку однакових понять та сутностей в інших наборах даних для семантичної гармонізації

Унікальні пари ключ-значення у DDF
Кожна з цих колекцій складається з пар ключ-значення. Кожна пара ключ-значення унікальна в цілому наборі даних.

Gapminder модерує цей процес, щоб переконатись, що люди, які беруть в ньому участь, знають, що вони роблять.
